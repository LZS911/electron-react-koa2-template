...
class Json extends BaseClass {
  public getApiData(swaggerJsonSrc: string) {
    ...
    // })
    // console.log(set);
    const { definitions } = swaggerObj;
    const { parameters } = swaggerObj;
  ...
  public handleSwaggerJsonFile(swaggerJsonSrc: string): ISwaggerJson {
    console.log(
      `start read swagger.json...;current src of find: ${swaggerJsonSrc}`
    );
  ...
  public generateApiMap(
    swaggerJson: ISwaggerJson
  ): Map<string, ISwaggerPaths[]> {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (CommonData.RequestMethod.includes(apiMethod)) {
          if (!currentApi[apiMethod].tags) {
            throw new Error(
              `Json: generateApiMap: there are no tags in api ${path}:${currentApi}:${apiMethod}`
            );
          }
