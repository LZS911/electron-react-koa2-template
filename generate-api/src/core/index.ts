...
class Core extends BaseClass {
  ...
  public generateApi() {
    ...
    write.writeFile(`${this.outputSrc}/common.enum.ts`, this.commonEnumBuffer);
    write.writeFile(
    ...
    for (const [key, value] of apiData.apiMap) {
      ...
      const className = CommonData.removeSpecialStr(key);
  ...
  public generateMethod(methods: ISwaggerPaths[]) {
      ...
      Object.keys(method).forEach((url) => {
        Object.keys(method[url]).forEach((requestMethod) => {
          if (CommonData.RequestMethod.includes(requestMethod)) {
            ...
            const { operationId } = method[url][requestMethod];
    ...
    let importListStr = '';
    for (const [key, value] of this.interfaceImportList) {
      importListStr += `
        import {
    ...
    if (
      ...
      if (body.schema.type === 'array') {
        if (
          ...
          const { isEnum, isInterface, name, isNormal } =
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (Object.keys(this.typeDictionary).includes(definitions[key].type)) {
        if (definitions[key].enum && this.validEnum(definitions[key].enum)) {
          const enumName = CommonData.removeSpecialStr(
            `${key[0].toUpperCase()}${key.slice(1)}Enum`
          );
          const enumBody = this.generateEnum(definitions[key].enum);
      ...
      if (definitions[key].allOf) {
        const properties = definitions[key].allOf.find(
          (item) => !!item.properties
        );
        if (!properties) {
          console.log(definitions[key]);
          throw new Error(
            'Core: generateCommonInterface: not find "properties" field'
          );
        }
        definitions[key].properties = properties.properties;
        definitions[key].allOf.forEach((item) => {
          if (item.$ref) {
            const { name } = this.transformDefinitionsInterface(item.$ref);
            extendsInterface.push(name);
          } else if (!item.properties) {
            console.log(definitions[key].allOf);
            console.warn(
              'Core: generateCommonInterface: maybe miss some fields'
            );
          }
        });
      }
        ...
        Object.keys(definitions[key].properties).forEach((prop) => {
          ...
          if (Object.keys(this.typeDictionary).includes(currentProp.type)) {
            currentInterfaceBody += `
              ...
              enumName || this.typeDictionary[currentProp.type]
            };
            `;
      ...
      this.commonInterfaceBuffer += template.interfaceTemplate(
        currentInterfaceName,
        currentInterfaceBody,
        extendsInterface.join(',')
      );
    }
    let importListStr = '';
    for (const [key, value] of this.commonInterfaceImport) {
      if (key === 'common.d') {
        continue;
      }
      const path =
      ...
      if (param.name === 'body' && param.in === 'body') {
        ...
        if (Object.keys(this.typeDictionary).includes(param.type)) {
          currentInterfaceBody += `
            ...
            enumName || this.typeDictionary[param.type]
          };
          `;
    ...
    Object.keys(props).forEach((prop, i) => {
      ...
      if (Object.keys(this.typeDictionary).includes(currentProps.type)) {
        body += `
          ...
          enumName || this.typeDictionary[currentProps.type]
        };
        `;
  ...
  private getTypeFromArray(
    ...
    if (array.items.$ref) {
      const { isEnum, isInterface, isNormal, name } =
  ...
  private getTypeFromObject(
    ...
    if (props.additionalProperties) {
      ...
      if (props.additionalProperties.type === 'array') {
        return `{
      ...
      if (props.additionalProperties.type === 'object') {
        return `{
    ...
    if (props.allOf) {
      const properties = props.allOf.find((item) => !!item.properties);
      const extendsType = [];
      props.allOf.forEach((item) => {
        if (item.$ref) {
          const { name } = this.transformDefinitionsInterface(item.$ref);
          this.addValue(interfaceImport, 'common.d', name);
          extendsType.push(name);
        } else if (!item.properties) {
          console.log(props.allOf);
          console.warn('Core: getTypeFromObject: maybe miss some fields');
        }
      });
      if (!properties) {
        return extendsType.join('&');
      }
